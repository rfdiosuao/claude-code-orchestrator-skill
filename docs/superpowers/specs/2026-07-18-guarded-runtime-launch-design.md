...
Event serialization uses an explicit field allowlist, origin-aware exact credential/endpoint-secret rejection plus pattern checks, a per-event size cap, symlink/reparse refusal, bounded cross-process locking, and a local hash chain. On Darwin, only the root-owned fixed system aliases `/etc -> /private/etc`, `/tmp -> /private/tmp`, and `/var -> /private/var` are accepted after descriptor-relative identity and target verification; every target component after that entry point still uses no-follow directory traversal. Audit-root identities query the containing volume's native case-sensitivity capability through a retained directory descriptor: case-sensitive volumes preserve exact path spelling, while case-insensitive volumes use a normalized case-folded identity so spelling variants cannot split failure state. An unavailable or invalid capability response fails closed. Detail values are never persisted; only allowlisted field names are retained. Prompt/task text cannot collide with fixed event vocabulary because it is neither serialized nor treated as an opaque audit secret. The 32-byte HMAC key lives under the workspace artifact root, not the installed skill directory. First use atomically establishes the key, an empty log, and an authenticated zero-event checkpoint. Once the key exists, a missing log or checkpoint is corruption and fails closed.
